// run this script with the path to the .docx file as an argument  (node isAccessible.js path/to/file.docx)
// Example: node isAccessible.js C:\Users\username\Downloads\file.docx
// If the file is not a Word document, the script will throw an error and exit.
// If the file is a Word document, the script will read the custom properties XML and print the value of the "isAccessible" property if it exists.
// If the "isAccessible" property is not defined, the script will print "No custom properties found in this document."  and exit.
// If the "isAccessible" property is defined, the script will print "The value of the isAccessible property is: true" and exit.
 ...
 * @return {Promise<Object>} A Promise that resolves to an object representing the custom properties.
 ...
 */
// Function to read custom properties XML
async function readCustomPropertiesXml(filePath) {
    try {
        ...
        const xmlContent = customXml.getData().toString('utf8');
...
const r = checkAccessibility(filePath);
r.then((a) => {
    console.log(a);
}).catch((e) => {
    console.log(e);
